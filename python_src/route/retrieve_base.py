    ...
    async def retrieve(self, query_str,time_out=10):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(self.api_url, json=params, timeout=aiohttp.ClientTimeout(total=time_out)) as response:
            ...
            for doc in results:
                ...
                node_dict['img_src'] = img_src_base if not doc.get('img_src') else doc.get('img_src')
                node_dict['thumbnail_src'] = img_src_base if not doc.get('thumbnail_src') else doc.get('thumbnail_src')
                node_dict['thumbnail'] = img_src_base if not doc.get('thumbnail') else doc.get('thumbnail')
                node_dict['author'] = "" if not doc.get('author') else doc.get('author')
                node_dict['iframe_src'] = first_frame_base if not doc.get('iframe_src') else doc.get('iframe_src')
